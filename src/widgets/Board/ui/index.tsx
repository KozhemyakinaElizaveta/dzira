...
export const Board = () => {
  ...
  const handleTaskDrop = (taskId: string, newStatus: string) => {
    ...
    updateTaskStatus(parseInt(taskId, 10), newStatus as 'Backlog' | 'InProgress' | 'Done');

    setGroupedTasks((prevTasks) => {
      const updatedTasks = { ...prevTasks };

      Object.keys(updatedTasks).forEach((status) => {
        updatedTasks[status] = updatedTasks[status].filter((task) => task.id !== parseInt(taskId, 10));
      });

      updatedTasks[newStatus].push(tasks.find((task) => task.id === parseInt(taskId, 10))!);
      ...
      <Flex w={'100%'} h={'100%'} py={'10px'} position="relative">
        ...
        {['Backlog', 'InProgress', 'Done'].map((status) => (
          <BoardColumn
            key={status}
            title={status}
            titleRus={status === 'Backlog' ? 'Бэклог' : status === 'InProgress' ? 'В процессе' : 'Готово'}
